"""Phase 5b — freeze the enriched model + export artifacts for audit.

Exports into data/model_artifacts/:
  model.json             native XGBoost binary (n_estimators locked to best_iteration + 1)
...
Phase 5a artifacts are preserved in data/model_artifacts/5a_baseline/.
"""
...
# 2. Find best iteration with the same early-stopped config as the review run.
...
best_iteration = int(probe.best_iteration)

# 3. Fit model locked to best iteration + 1 (XGBoost best_iteration is
# 0-indexed; the early-stopped model predicts with trees 0..best_iteration).
...
# 4. Save metadata artifact.
# Operational threshold = exact max-F1 point from the PR curve. Derive it (never
# hardcode a rounded literal — it would shift the cutoff by ~1e-4 and silently
...
plt.axvline(
    x=metadata["metrics"]["recall"],
    color="r",
    linestyle="--",
    label=f"Recall @ {threshold:.4f} ({metadata['metrics']['recall'] * 100:.0f}%)",
)
plt.xlabel("Recall")
plt.ylabel("Precision")
plt.title("Precision-Recall Curve (Phase 5b Enriched)")
plt.legend()
plt.tight_layout()
...
plt.title("ROC Curve (Phase 5b Enriched)")
plt.legend()
plt.tight_layout()
...
plt.title(f"Confusion Matrix (Thresh = {threshold:.4f})")
plt.tight_layout()
...
plt.title("XGBoost Feature Gain (Phase 5b Enriched)")
plt.tight_layout()
plt.savefig(os.path.join(artifact_dir, "feature_importance.png"))
plt.close()

print(f"Phase 5b artifacts successfully exported to {artifact_dir}/")
print(f"best_iteration={best_iteration} num_trees={n_trees} "
